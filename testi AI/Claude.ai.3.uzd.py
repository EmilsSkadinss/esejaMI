...
def calculate_metrics(df, mass=0.2):
    ...
    Returns:
        tuple: DataFrames ar aprēķināto spēku un augstumu
    """
    ...
    velocity = cumulative_trapezoid(df['total_acceleration'], df['time'], initial=0)
    
    # Aprēķinam augstumu, integrējot ātrumu
    height = cumulative_trapezoid(velocity, df['time'], initial=0)
    df['height'] = height
